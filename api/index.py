"""
Leads In The Pipe — email backend (Vercel serverless function).

This mirrors backend/main.py but is deployed as a Vercel Python serverless
function so the whole site lives in one Vercel project. The frontend posts to
the same origin under /api/* (see vercel.json rewrites).

Configure via Vercel project Environment Variables:
  SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASSWORD, SMTP_USE_TLS,
  MAIL_FROM, MAIL_TO, ALLOWED_ORIGINS

If SMTP is not configured the message is logged instead of sent.
"""
...
# --------------------------------------------------------------------------- #
# Routes (paths include the /api prefix to match Vercel rewrites)             #
# --------------------------------------------------------------------------- #
